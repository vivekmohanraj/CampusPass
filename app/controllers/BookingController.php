...
if ($action === 'book' && $_SERVER['REQUEST_METHOD'] === 'POST' && $user_id) {
    ...
    if ($already) {
        die('You have already booked a ticket for this event. <a href="../views/student_events.php">Back to Events</a>');
    }
    // Check event status and tickets remaining
    $event = $eventModel->findById($event_id);
    if (!$event || $event['status'] === 'Completed') {
        die('Event is completed or not found. <a href="../views/student_events.php">Back to Events</a>');
    }
    ...
    if ($max && $max > 0 && $tickets_count >= $max) {
        die('Tickets sold out. <a href="../views/student_events.php">Back to Events</a>');
    }
    if ($price == 0) {
        $payment_status = 'Free';
        $ticketModel->create(['event_id' => $event_id, 'user_id' => $user_id, 'payment_status' => $payment_status]);
        header('Location: ../views/booking_confirmation.php?event_id=' . $event_id);
        exit;
    } else {
        // Paid event: payment_id must be present
        if (!$payment_id) {
            die('Payment not completed. <a href="../views/student_events.php">Try again</a>');
        }
        // Save payment record
